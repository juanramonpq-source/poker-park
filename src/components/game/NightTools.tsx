...
import { hasJackBox, jackBoxCards, jokerAvailable, jokerOptions } from "@/lib/game/night-tools";
...
export function NightJackBox({ game, locked }: { game: GameState; locked: boolean }) {
  ...
  const box = jackBoxCards(game);
  const title = game.challenge === "night" ? "Caseta de guardia" : "Caseta de Jotas";
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    <Dialog.Trigger className="night-jack-trigger" disabled={locked}>
      <BriefcaseBusiness aria-hidden /><strong>{title}</strong><span>J · {box.length}</span>
    </Dialog.Trigger>
    <Dialog.Portal>
      ...
      <Dialog.Content className="game-help-dialog">
        ...
        <Dialog.Description>Solo Jotas. Al llegar a tu mano, descansan aquí y robas otra carta si queda mazo. {hasMirrorRules(game) ? "Puedes colgarlas en las Sillas antes de levantar la torre." : "Puedes colocarlas en las Sillas cuando la torre esté lista."} También puedes seleccionar una y pulsar Cambiar para recibir una carta de la Entrada. El cambio consume una maniobra; después colocas la carta recibida.{game.storm ? " Los cierres por tormenta siguen vigentes." : ""}</Dialog.Description>
...
export function NightJokerPicker({ game, onClose }: { game: GameState; onClose: () => void }) {
  ...
  if (game.challenge !== "night" || !hasJackBox(game) || game.difficulty !== "easy") return null;
